...
const StagnantWater = () => {
  const [waterFeed, setWaterFeed] = useState([]);
    ...
    const fetchData = async () => {
      try {
        const response = await axios.post('http://localhost:8080/api/adminData/getFeed', {
          selectedFeedValue: 'Stagnent Water'
        });
        setWaterFeed(response.data);
      } catch (error) {
  ...
  return (
    <div className="Streetlight">
      <h2>Water related issues</h2>
      {waterFeed.map((item, index) => {
